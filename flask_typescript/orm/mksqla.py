...
PY_PREAMBLE = Template(
    """from __future__ import annotations
{% for mod, name in pyimports %}
{%- if not mod.startswith('sqlalchemy') and name != 'get_args' -%}
from {{mod}} import {{name}}
{% endif %}
{%- endfor -%}
...
def get_py_template() -> str:
    return read_text("flask_typescript.orm", "pydantic.py.jinja")
...
class ColumnInfo:
    ...
    @property
    def use_field(self) -> bool:
        return self.column_name != self.name

    def pyfield(self) -> str:
        ...
        if d and d.isdigit():
            ...
            default = "None"
        if use_field:
            return f' = Field(alias="{self.name}", default={default})'
...
class ModelMaker:
    def __init__(
        ...
        aspydantic: bool = False,
    ):
        if ns is None:
            ns = NS
        self.with_tablename = with_tablename
        self.abstract = abstract
        self.template = Template(get_py_template() if aspydantic else get_template())
        self.base = base
        self.ns = ns
        self.throw = throw
        self.aspydantic = aspydantic
            ...
            for i in indexes:
                if len(i.columns) == 1:
                    if c.name in i.columns:
                        ...
                        indexes.remove(i)
                        break
            d = self.fixup_column(d, pyimports)
            columns.append(d)
    ...
    def fixup_column(
        ...
        if not self.aspydantic:
            return col
        if col.column_name == "schema":
            pyimports.add(("pydantic", "Field"))
            return replace(col, column_name=col.column_name + "_")
        return col
    ...
    def get_enum_name(self, col: Column[Any], enums: dict[tuple[str, ...], str]) -> str:
        name = f"{col.key.title()}_Enum"
        return self.unique_name(name, set(enums.values()))

    def get_set_name(self, col: Column[Any], sets: dict[tuple[str, ...], str]) -> str:
        name = f"{col.key.title()}_Set"
        return self.unique_name(name, set(sets.values()))
    ...
    def get_preamble(self) -> Template:
        return PY_PREAMBLE if self.aspydantic else PREAMBLE
